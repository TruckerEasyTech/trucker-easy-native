    ...
    with httpx.Client(timeout=30.0) as client:
        for url in TPIMS_URLS:
            try:
                ...
                if isinstance(data, list):
                    parsed = parse_tpims_dynamic(data, url)
                    # TPIMS static metadata may be separate; geo match happens via static feed in production.
                    # For dynamic-only URLs, siteId matching would need static companion feed.
                    print(f"  {len(parsed)} TPIMS dynamic entries (geo match requires lat/lon from static feed)")
        ...
        if OHGO_API_KEY:
            try:
                ...
                resp = client.get(
                    OHGO_BASE,
                    headers={"Authorization": f"Bearer {OHGO_API_KEY}", "Accept": "application/json"},
                )
                resp.raise_for_status()
