-- Idempotent apply for remote (weigh_station_reports uses updated_at, not reported_at).

alter table public.poi_places drop constraint if exists poi_places_osm_type_check;
update public.poi_places set osm_type = 'external' where osm_type = 'feature';
